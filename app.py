...
def asset_weights_summary_html(asset1: str, weight1: float, asset2: str, weight2: float, risk_free_weight: float) -> str:
    ...
    if risk_free_weight > 1e-9:
        ...
        capital_copy = f"Under standard portfolio theory, these are the optimal weights of the two risky assets and they sum to 100.00%. {abs(risk_free_weight):.2%} at the risk-free rate to lever that risky portfolio."
    else:
    ...
    <div class="allocation-summary">
        ...
        <p class="allocation-summary-copy">These are the optimal two-asset portfolio weights computed from the entered expected returns, standard deviations, and correlation coefficient.</p>
        ...
        <div class="allocation-chip-row" style="margin-top:0.55rem;">
            ...
            <span class="allocation-chip">Risk-Free Allocation: 0.00%</span>
        </div>
    </div>
    """
...
def _builder_optimal_total_risky_for_mix(
    ...
    """
    Kept for compatibility with the rest of the app, but under the standard
    fully invested two-risky-asset setup the total risky allocation is always 1.
    """
    return np.ones_like(risky_mix_excess, dtype=float)
...
def _builder_candidate_portfolios_for_gamma(
    ...
    """
    Standard portfolio theory setup for two risky assets:
    - fully invested in the two risky assets
    - no borrowing or lending through the risk-free asset
    - weights sum to 1 exactly
    """
    total_risky = np.ones_like(risky_mix_grid, dtype=float)
    x1 = np.array(risky_mix_grid, dtype=float)
    x2 = 1.0 - x1
    risk_free_weight = np.zeros_like(risky_mix_grid, dtype=float)

    portfolio_returns = np.array(risky_mix_returns, dtype=float)
    portfolio_variances = np.maximum(np.array(risky_mix_variances, dtype=float), 0.0)
    portfolio_risks = np.sqrt(portfolio_variances)
    portfolio_excess_returns = portfolio_returns - rf
    portfolio_esg = np.array(risky_mix_esg, dtype=float)
...
def compute_builder_result(
    ...
    total_weight = max(opt_x1 + opt_x2, 1e-12)
    opt_x1 = opt_x1 / total_weight
    opt_x2 = opt_x2 / total_weight
    opt_rf_weight = 0.0
    opt_risky_mix_weight1 = opt_x1
    opt_risky_mix_weight2 = opt_x2
...
def render_builder_popup() -> None:
    ...
    with outer_mid:
        ...
        with popup:
            ...
            display_weight1 = float(np.clip(result.get("opt_w1", 0.0), 0.0, 1.0))
            display_weight2 = float(np.clip(result.get("opt_w2", 0.0), 0.0, 1.0))
            total_risky_exposure = max(display_weight1 + display_weight2, 0.0)
            ...
            with summary_tab:
                ...
                with input_c1:
                    st.markdown(
                        result_tile(
                            f'{safe_asset1_name} Portfolio Weight',
                ...
                with input_c2:
                    st.markdown(
                        result_tile(
                            f'{safe_asset2_name} Portfolio Weight',
            ...
            with asset_weights_tab:
                ...
                st.caption("This tab shows the standard fully invested two-asset portfolio weights. The two weights sum to 100% and there is no borrowing or lending in this calculation.")
